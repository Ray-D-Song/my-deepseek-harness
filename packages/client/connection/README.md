...
An optional `cloudflareAccess` Connection setting can bootstrap the same DSH cookie after Cloudflare Access has authenticated the browser. Set `teamDomain` to the HTTPS Team Domain and `audience` to the Access Application Audience Tag. The Host validates the `Cf-Access-Jwt-Assertion` header against the Team Domain JWKS, issuer, audience, expiry, not-before time, key id, and `RS256`; it never logs the token. A valid request to clean `GET /` receives the ordinary DSH HttpOnly cookie and a 303 redirect back to `/`. The existing `?token=...` exchange remains available as a recovery path. Leave the setting absent to disable this integration.
